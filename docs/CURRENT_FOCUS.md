...
M0 foundation is complete and the exit gate passed on this machine:
...
- OpenAI-compatible Chat Completions `fetch` adapter, URL normalize, error taxonomy, test connection
- Structured-output fallback runner (json_schema → json_object → extract/validate/repair)
...
1. Add Source entity + repository and DB migration (new Dexie version; keep v1 chain).
